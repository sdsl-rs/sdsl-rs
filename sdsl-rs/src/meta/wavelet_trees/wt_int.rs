...
fn get_header_replacements(
    ...
    replacements.insert(
        "#define WT_INT_TEMPLATE sdsl::bit_vector, sdsl::bit_vector::rank_1_type, sdsl::bit_vector::select_1_type, sdsl::bit_vector::select_0_type".to_string(),
        format!(
            "#define WT_INT_TEMPLATE {}",
            parameters_c_code.join(", ")
        ),
    );

    replacements.insert(
        "#define WT_INT_ID _id".to_string(),
...
impl common::Path for WtIntMeta {
    fn path(&self) -> String {
        "sdsl::wavelet_trees::WtInt".to_string()
    }
}

impl common::Code for WtIntMeta {
    fn c_code(&self, parameters_c_code: &Vec<String>) -> Result<String> {
        let parameters = self.parameters_definitions();
        let parameters_c_code = common::c_sorted_parameters(&parameters_c_code, &parameters)?;
        Ok(format!("sdsl::wt_int<{}>", parameters_c_code.join(", ")))
    }
}

impl common::Parameters for WtIntMeta {
